...
public class VisualRaids : MonoBehaviour
{
    ...
    {
        string ru = language == "ru" ? "—“–Œ»“‹ –≈À‹—€ <Color=red>[…]" : "BUILD RAILS [Q]";
        if (!ismake)
        {
            ru = language == "ru" ? "—“–Œ»“‹ –≈À‹—€ <Color=red>[…]" : "MAKE READ <Color=red>[Q]";
            return ru ;
        }
        else
        {
            {
                ru = ru = language == "ru" ? "Œ—“¿ÕŒ¬»“‹ —“–Œ… ” <Color=red>[…]" : "STOP CONSTRUCTION <Color=red>[Q]";
                return ru;
            }
        }
    }
